...
function parseJsonFromText(raw: string): { title?: string; content?: string } | null {
  if (!raw) return null;

  function tryExtract(obj: any): { title: string; content: string } | null {
    ...
    if (typeof obj.content === 'string') {
      const inner = parseJsonFromText(obj.content);
      if (inner?.title && inner?.content) {
        return inner as { title: string; content: string };
      }
    }

    // Choices array format
    if (Array.isArray(obj.choices) && obj.choices[0]?.message?.content) {
      return parseJsonFromText(obj.choices[0].message.content) as any;
    }
