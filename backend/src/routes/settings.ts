...
import { PRESETS, DEFAULT_SETTINGS, PRESET_METADATA } from '../config/presets';
...
router.post('/reset', async (req: AuthedRequest, res) => {
  try {
    const user = await User.findById(req.userId);
    if (!user) return res.status(404).json({ error: 'USER_NOT_FOUND' });
...
router.post('/apply-preset', async (req: AuthedRequest, res) => {
...
router.delete('/data', async (req: AuthedRequest, res) => {
  try {
